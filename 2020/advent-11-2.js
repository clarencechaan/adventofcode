...
let grid = [];
for (const row of input) grid.push([...row]);

function printGrid() {
  console.log(grid.map((arr) => arr.join("")).join("\n"));
}

function getNumOccupied(row, col) {
  let occupied = 0;
  let i;
  let j;
  for (i = 1; row - i >= 0 && grid[row - i][col] === "."; i++);
  if (grid[row - i]?.[col] === "#") occupied++;

  for (i = 1; row + i <= n - 1 && grid[row + i][col] === "."; i++);
  if (grid[row + i]?.[col] === "#") occupied++;

  for (j = 1; col - j >= 0 && grid[row][col - j] === "."; j++);
  if (grid[row][col - j] === "#") occupied++;

  for (j = 1; col + j <= m - 1 && grid[row][col + j] === "."; j++);
  if (grid[row][col + j] === "#") occupied++;

  for (
    i = 1, j = 1;
    row - i >= 0 && col - j >= 0 && grid[row - i][col - j] === ".";
    i++, j++
  );
  if (grid[row - i]?.[col - j] === "#") occupied++;

  for (
    i = 1, j = 1;
    row - i >= 0 && col + j <= m - 1 && grid[row - i][col + j] === ".";
    i++, j++
  );
  if (grid[row - i]?.[col + j] === "#") occupied++;
  ...
  for (
    i = 1, j = 1;
    row + i <= n - 1 && col - j >= 0 && grid[row + i][col - j] === ".";
    i++, j++
  );
  if (grid[row + i]?.[col - j] === "#") occupied++;
  ...
  for (
    i = 1, j = 1;
    row + i <= n - 1 && col + j <= m - 1 && grid[row + i][col + j] === ".";
    i++, j++
  );
  if (grid[row + i]?.[col + j] === "#") occupied++;
...
for (let done = false; !done; ) {
  ...
  let next = [];
  for (const row of grid) next.push([...row]);

  for (let i = 0; i < n; i++)
    for (let j = 0; j < m; j++) {
      if (grid[i][j] === "L" || grid[i][j] === "#") {
        let occupied = getNumOccupied(i, j);
