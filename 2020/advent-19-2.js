...
rules.sort((a, b) => (a[0] > b[0] ? 1 : -1));
rules = rules.map(([, rule]) => rule);

function convertRulesToStr(ruleID = 0) {
  let arr = rules[ruleID];
  for (let i = 0; i < arr.length; i++)
    for (let j = 0; j < arr[i].length; j++)
      if (typeof arr[i][j] === "number")
        arr[i][j] = arr[i][j] === ruleID ? arr : convertRulesToStr(arr[i][j]);
  return arr;
}

convertRulesToStr();

function isMessageValid(message, msgIdx = 0, rule = rules[0][0]) {
  for (let i = 0; i < rule.length; i++) {
    if (typeof rule[i] === "string") {
      if (message[msgIdx] !== rule[i]) return false;
      msgIdx++;
    } else
      return rule[i].some((next) =>
        isMessageValid(message, msgIdx, [...next, ...rule.slice(i + 1)])
      );
  }
...
let output = 0;
for (const message of messages) if (isMessageValid(message)) output++;
console.log(output);
