let input = `6 * (2 + 4 * (3 + 7 * 8 + 4) * 8 + 3 + (9 + 8 + 4 + 9)) * 8 + ((2 * 5 * 9 + 7 + 4 * 3) + (8 + 3 + 7) + (7 + 2) * 8 * 8 + 9)
...
function strExprToArr(str) {
  let arr = [];
  let idx;
  let p = 0;
  for (let i = 0; i < str.length; i++) {
    if (!p && "0" <= str[i] && str[i] <= "9") arr.push(parseInt(str[i]));
    else if (!p && (str[i] === "*" || str[i] === "+")) arr.push(str[i]);
    else if (str[i] === "(") {
      p++;
      if (p === 1) idx = i;
    } else if (str[i] === ")") {
      p--;
      if (p === 0) arr.push(strExprToArr(str.slice(idx + 1, i)));
    }
  }
  return arr;
}
...
function evaluate(expr) {
  ...
  for (let i = 1; i < expr.length; i += 2)
    if (expr[i] === "+") {
      expr.splice(i - 1, 3, evaluate(expr[i - 1]) + evaluate(expr[i + 1]));
      i -= 2;
    }
  let val = evaluate(expr[0]);
  for (let i = 1; i < expr.length; i += 2) val *= evaluate(expr[i + 1]);
  return val;
}

let output = 0;
for (const str of input) output += evaluate(strExprToArr(str));
console.log(output);
