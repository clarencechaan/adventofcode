...
function splitLine(line) {
  ...
  for (let i = 0; i < line[1].length; i++) {
    ...
    let count = parseInt(line[1][i][line[1][i].indexOf(" ") - 1]);
...
function numOfBags(parent = "shiny gold") {
  let result = 0;
  for (let i = 0; i < bagMap[parent].length; i += 2)
    result +=
      bagMap[parent][i] + bagMap[parent][i] * numOfBags(bagMap[parent][i + 1]);
  return result;
}

let output = numOfBags();
console.log(output);
