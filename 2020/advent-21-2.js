...
input = input.split("\n").map((str) => {
  let arr = str.split(" (contains ");
  arr[0] = new Set(arr[0].split(" "));
  arr[1] = new Set(arr[1].slice(0, -1).split(", "));
  return arr;
});

input.sort((a, b) => (a[1].size > b[1].size ? 1 : -1));

let allergenList = new Set();
for (const [, allergens] of input)
  for (const alrg of allergens) allergenList.add(alrg);

allergenList = [...allergenList];

function getIntersection(set1, set2) {
  let intersection = new Set();
  for (const elem of set1) if (set2.has(elem)) intersection.add(elem);
  return intersection;
}

let possible = [];
for (const alrg of allergenList) {
  let idx = possible.length;
  possible.push({ allergen: alrg });
  for (const [ingredients, allergens] of input) {
    if (!allergens.has(alrg)) continue;
    if (!possible[idx].ingredient)
      possible[idx].ingredient = new Set(ingredients);
    else
      possible[idx].ingredient = getIntersection(
        possible[idx].ingredient,
        ingredients
      );
  }
}
...
for (let i = 0; i < possible.length; i++)
  possible[i].ingredient = [...possible[i].ingredient];

function findAllergenIngredients(pIdx = 0, mapping = new Set()) {
  if (pIdx === possible.length) return mapping;
  let { ingredient } = possible[pIdx];
  for (let i = 0; i < ingredient.length; i++) {
    if (mapping.has(ingredient[i])) continue;
    mapping.add(ingredient[i]);
    let result = findAllergenIngredients(pIdx + 1, mapping);
    if (result) return result;
    mapping.delete(ingredient[i]);
  }
}

let allergenIngredients = findAllergenIngredients();

let output = [...allergenIngredients].map((ingr, idx) => [
  ingr,
  possible[idx].allergen,
]);
output.sort((a, b) => (a[1] > b[1] ? 1 : -1));
output = output.map(([ingr]) => ingr).join(",");
