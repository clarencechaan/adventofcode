...
for (let i = 0; i < input.length; i++) {
  let arr = input[i]
    .replaceAll("\n", " ")
    .split(" ")
    .map((str) => [str.slice(0, 3), str.slice(4)]);
  let map = {};
  for (const [property, value] of arr)
    if (property !== "cid") map[property] = value;
  input[i] = map;
}

let output = 0;
for (let { byr, iyr, eyr, hgt, hcl, ecl, pid } of input) {
  ...
  let hgtNumber = parseInt(hgt.slice(0, -2));
  if (!hgtNumber) continue;
  if (hgt.slice(-2) === "cm" && (hgtNumber < 150 || hgtNumber > 193)) continue;
  if (hgt.slice(-2) === "in" && (hgtNumber < 59 || hgtNumber > 76)) continue;

  if (hcl[0] !== "#") continue;
  if (hcl.slice(1).length !== 6) continue;
  for (let i = 1; i <= 6; i++)
    if (!("0" <= hgt[i] && hgt[i] <= "9") && !("a" <= hgt[i] && hgt[i] <= "z"))
      continue;
