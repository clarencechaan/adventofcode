let input = `plaid magenta bags contain 2 clear lavender bags, 3 clear teal bags, 4 vibrant gold bags.
...
function splitLine(line) {
  ...
  let parent = line[0].slice(0, -5);
  line[1] = line[1].split(", ");

  let result = [parent];
  for (let i = 0; i < line[1].length; i++) {
    if (line[1][i].includes("no other bags")) continue;
    let bagName = line[1][i].split("");
    bagName = bagName.slice(
      line[1][i].indexOf(" ") + 1,
      bagName.lastIndexOf(" ")
    );
    bagName = bagName.join("");
    let count = line[1][i].indexOf(" ");
    result.push(count, bagName);
  }
  return result;
}

for (let i = 0; i < input.length; i++) input[i] = splitLine(input[i]);
...
for (const arr of input) {
  ...
  for (let i = 1; i < arr.length; i += 2)
    bagMap[parent].push(arr[i], arr[i + 1]);
}

function canHold(parent, child = "shiny gold") {
  if (parent === child) return false;
  for (let i = 1; i < bagMap[parent].length; i += 2)
    if (bagMap[parent][i] === child || canHold(bagMap[parent][i])) return true;
  return false;
}

let output = 0;
for (const parent in bagMap) if (canHold(parent)) output++;
console.log(output);
