...
function getRotatedSquare(sqr) {
  let result = [];
  for (let i = 0; i < 10; i++) result.push([]);

  for (let i = 0; i < 10; i++)
    for (let j = 0; j < 10; j++) result[j][9 - i] = sqr[i][j];
...
function getFlippedSquare(sqr) {
  let result = [];
  for (let i = 0; i < 10; i++) result.push([...sqr[i]].reverse());
  return result;
}
...
findArrangement();
let output =
  arr[0][0][0] * arr[0][n - 1][0] * arr[n - 1][n - 1][0] * arr[n - 1][0][0];
console.log(output);
