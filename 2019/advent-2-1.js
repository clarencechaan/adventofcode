...
for (let i = 0; i < input.length && input[i] !== 99; i += 4) {
  const idx1 = input[i + 1];
  const idx2 = input[i + 2];
  const idx3 = input[i + 3];
  if (input[i] === 1) input[idx3] = input[idx1] + input[idx2];
  else if (input[i] === 2) input[idx3] = input[idx1] * input[idx2];
}
