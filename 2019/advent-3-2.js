...
input = input
  .split("\n")
  .map((str) => str.split(",").map((str) => [str[0], parseInt(str.slice(1))]));
...
for (let [direction, distance] of input[0]) {
  while (distance) {
    steps++;
    switch (direction) {
      case "R":
...
for (let [direction, distance] of input[1]) {
  while (distance) {
    steps++;
    switch (direction) {
      case "R":
    ...
    if (
      path[x + "," + y] &&
      (!intersections[x + "," + y] ||
        intersections[x + "," + y] > path[x + "," + y] + steps)
    )
      intersections[x + "," + y] = path[x + "," + y] + steps;
    distance--;
  }
...
let output = Infinity;
for (const key in intersections) output = Math.min(output, intersections[key]);
console.log(output);
