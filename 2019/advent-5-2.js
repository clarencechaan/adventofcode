...
for (let i = 0; i < input.length && input[i] !== 99; ) {
  const opcode = input[i].toString().slice(-2);
  const modes = input[i].toString().slice(0, -2).split("").reverse().join("");
  const instruction = opcode[1] || opcode[0];
  const parameters = input.slice(i + 1, i + 4);
  switch (instruction) {
    case "1":
      input[parameters[2]] =
        (modes[0] === "1" ? parameters[0] : input[parameters[0]]) +
        (modes[1] === "1" ? parameters[1] : input[parameters[1]]);
      i += 4;
      break;
    case "2":
      input[parameters[2]] =
    ...
    case "3":
      ...
      break;
    case "4":
      console.log(input[parameters[0]]);
      i += 2;
      break;
    case "5":
      if (
        (modes[0] === "1" && parameters[0] !== 0) ||
        (modes[0] !== "1" && input[parameters[0]] !== 0)
      )
        i = modes[1] === "1" ? parameters[1] : input[parameters[1]];
      else i += 3;
      break;
    case "6":
      if (
    ...
    case "7":
      if (
        ...
        input[parameters[2]] = 1;
      else input[parameters[2]] = 0;
      i += 4;
      break;
    case "8":
      if (
