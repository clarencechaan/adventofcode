...
function getNumTokens(machine) {
  ...
  const memo = {};
  function recursion(x = 0, y = 0, aPress = 0, bPress = 0) {
    if (aPress > 100 || bPress > 100) return Infinity;
    if (x > prizeX || y > prizeY) return Infinity;
    if (x === prizeX && y === prizeY) return 0;

    const str = `${x},${y},${aPress},${bPress}`;
    if (memo[str] !== undefined) return memo[str];

    const pressA =
      3 + recursion(x + axDistance, y + ayDistance, aPress + 1, bPress);
    const pressB =
      1 + recursion(x + bxDistance, y + byDistance, aPress, bPress + 1);

    const numTokens = Math.min(pressA, pressB);
    memo[str] = numTokens;
    return numTokens;
  }

  let numTokens = recursion();
  if (numTokens === Infinity) numTokens = 0;
  return numTokens;
}
