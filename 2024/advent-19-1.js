...
function canMake(design, cursor = 0) {
  if (cursor === design.length) return true;

  for (const towel of towels) {
    const length = towel.length;
    if (
      design.slice(cursor, cursor + length) === towel &&
      canMake(design, cursor + length)
    )
      return true;
  }
...
const result = designs.filter((design) => canMake(design)).length;
console.log(result);
