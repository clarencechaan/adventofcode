...
input = input
  ...
  .map(([buttonA, buttonB, prize]) => {
    ...
    prize = prize
      .slice(7)
      .split(", ")
      .map((str) => +str.slice(2) + 10000000000000);

    return [buttonA, buttonB, prize];
  });

function getNumTokens(machine) {
  const [[axDistance, ayDistance], [bxDistance, byDistance], [prizeX, prizeY]] =
    machine;

  const bPresses =
    (prizeY * axDistance - prizeX * ayDistance) /
    (byDistance * axDistance - bxDistance * ayDistance);

  const aPresses = (prizeX - bPresses * bxDistance) / axDistance;

  if (bPresses % 1 !== 0 || aPresses % 1 !== 0) return 0;
  return aPresses * 3 + bPresses;
}

let result = 0;
for (const machine of input) result += getNumTokens(machine);
console.log(result);

// aPresses * axDistance + bPresses * bxDistance = prizeX
// aPresses * ayDistance + bPresses * byDistance = prizeY

// find aPresses and bPresses

// isolate aPresses in each equation

// aPresses = (prizeX - bPresses * bxDistance) / axDistance
// aPresses = (prizeY - bPresses * byDistance) / ayDistance

// substitute aPresses and isolate bPresses

// (prizeX - bPresses * bxDistance) / axDistance = (prizeY - bPresses * byDistance) / ayDistance
// (prizeX - bPresses * bxDistance) * ayDistance = (prizeY - bPresses * byDistance) * axDistance
// prizeX * ayDistance - bPresses * bxDistance * ayDistance = prizeY * axDistance - bPresses * byDistance * axDistance
// bPresses * byDistance * axDistance - bPresses * bxDistance * ayDistance = prizeY * axDistance - prizeX * ayDistance
// bPresses * (byDistance * axDistance - bxDistance * ayDistance) = prizeY * axDistance - prizeX * ayDistance
// bPresses = (prizeY * axDistance - prizeX * ayDistance) / (byDistance * axDistance - bxDistance * ayDistance)
