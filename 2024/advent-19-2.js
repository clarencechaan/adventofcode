...
const memo = {};
function getNumWays(design, cursor = 0) {
  if (cursor === design.length) return 1;

  const str = design + cursor;
  ...
  for (const towel of towels) {
    ...
    if (design.slice(cursor, cursor + length) === towel)
      ways += getNumWays(design, cursor + length);
  }
...
const result = designs.reduce((sum, design) => sum + getNumWays(design), 0);
console.log(result);
