let input = `[[],[],[[],10,[[7,0,1,1,10],9,6],[1,[4,9,1],6,[4,6,0]],[0,3,0]],[1,[10,[7,4,3,4],[]],[2,2],7,[[10],5,3]]]
...
function strToArr(str) {
  ...
  for (let i = 1; i <= str.length - 1; i++) {
    if (str[i] === "[") val++;
    if (val > 0) substr += str[i];
    if (!val) {
      if (substr) {
        arr.push(strToArr(substr));
        substr = "";
      }
      if (str[i] >= "0" && str[i] <= "9") {
        if (str[i - 1] === "[" || str[i - 1] === ",") arr.push(str[i]);
  ...
  for (let i = 0; i < arr.length; i++)
    if (typeof arr[i] === "string") arr[i] = parseInt(arr[i]);
  return arr;
}

input = input.map((str) => strToArr(str));
...
function isOrderCorrect(left, right) {
  ...
  if (typeof left === "number" && typeof right === "object")
    return isOrderCorrect([left], right);
  if (typeof left === "object" && typeof right === "number")
    return isOrderCorrect(left, [right]);
  for (let i = 0; i < Math.min(left.length, right.length); i++) {
    let correct = isOrderCorrect(left[i], right[i]);
    if (correct === true) return true;
    else if (correct === false) return false;
  }
...
input.sort((a, b) => (isOrderCorrect(a, b) ? -1 : 1));
...
for (let i = 0; i < input.length; i++)
  if (
    ...
    decoderIndices.push(i + 1);

console.log(decoderIndices[0] * decoderIndices[1]);
