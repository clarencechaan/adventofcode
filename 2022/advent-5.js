...
for (let i = 0; i < 9; i++)
  for (let j = 2; j < input.length; j++)
    if (input[j][i * 4 + 1] !== " ") stacks[i].push(input[j][i * 4 + 1]);
...
// part 1
// for (let [count, from, to] of input) {
//   from--;
//   to--;
...
for (let [count, from, to] of input) {
  ...
  to--;
  let lift = [];
  for (let i = 0; i < count; i++) lift.push(stacks[from].pop());
  lift.reverse();
  stacks[to] = stacks[to].concat(lift);
}

let output = "";
for (const stack of stacks) output += stack.pop();
