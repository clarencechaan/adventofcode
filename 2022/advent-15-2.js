let input = `Sensor at x=346753, y=3974683: closest beacon is at x=527500, y=3570474
...
input = input
  ...
  .map((str) => str.split(", "))
  .map((arr) => ({
    sx: parseInt(arr[0].slice(arr[0].indexOf("x=") + 2)),
    sy: parseInt(arr[1].slice(2, arr[1].indexOf(":"))),
    bx: parseInt(arr[1].slice(arr[1].indexOf("x=") + 2)),
    by: parseInt(arr[2].slice(2)),
  }));
...
function isCoordPossible(x, y) {
  if (x < 0 || x > 4000000 || y < 0 || y > 4000000) return false;
  return input.every(
    (sensor) => Math.abs(y - sensor.sy) + Math.abs(x - sensor.sx) > sensor.dist
  );
}

function traceBorder(sensor) {
  let x = sensor.sx;
  let y = sensor.sy - sensor.dist - 1;
  while (y < sensor.sy) {
    x++;
    y++;
    if (isCoordPossible(x, y)) return [x, y];
  }
  while (x > sensor.sx) {
    x--;
    y++;
...
for (const sensor of input) {
  let coords = traceBorder(sensor);
  if (coords) {
    console.log(coords);
    console.log(coords[0] * 4000000 + coords[1]);
    return;
  }
}
