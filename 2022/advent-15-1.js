...
let row = 2000000;
let output = 0;
for (let i = minX; i <= maxX + maxDist; i++) {
  if (
    input.every((sensor) => sensor.bx !== i || sensor.by !== row) &&
    input.some(
      (sensor) =>
        Math.abs(row - sensor.sy) + Math.abs(i - sensor.sx) <= sensor.dist
    )
  )
    output++;
}
