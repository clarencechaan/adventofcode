...
function getMinimumCubesOfGame(idx) {
  let min = { red: 0, green: 0, blue: 0 };
  for (const bag of input[idx])
    for (const [count, colour] of bag)
      min[colour] = Math.max(min[colour], count);
  return min;
}

function getPower(min) {
  return min.red * min.green * min.blue;
}

let output = 0;
for (let i = 0; i < input.length; i++) {
  let min = getMinimumCubesOfGame(i);
  output += getPower(min);
}
console.log(output);
