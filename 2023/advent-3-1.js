...
function isNumberAdjacent(num, row, col) {
  for (let i = row - 1; i <= row + 1; i++)
    for (let j = col - 1; j <= col + num.length; j++)
      if (
        input[i]?.[j] &&
        input[i][j] !== "." &&
...
for (let row = 0; row < input.length; row++) {
  ...
  for (let col = 0; col <= line.length; col++) {
    if ("0" <= line[col] && line[col] <= "9") num += line[col];
    else if (num) {
      if (isNumberAdjacent(num, row, col - num.length)) output += parseInt(num);
      num = "";
    }
  }
}
