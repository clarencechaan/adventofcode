let input = `.......................661.........................485..565.......344.......325.....................................841.....725.............
...
function getNumberAt(row, col) {
  let num = "";
  for (let i = col; "0" <= input[row][i] && input[row][i] <= "9"; i++)
    num += input[row][i];
  if (num)
    while ("0" <= input[row][col - 1] && input[row][col - 1] <= "9") {
      num = input[row][col - 1] + num;
      col--;
    }
  if (num) return [num, row, col].join(",");
  return null;
}

function getGearRatio(row, col) {
  let nums = new Set();
  for (let i = row - 1; i <= row + 1; i++)
    for (let j = col - 1; j <= col + 1; j++) {
      const num = getNumberAt(i, j);
      if (num) nums.add(num);
    }
  if (nums.size === 2) {
    nums = [...nums].map((str) => parseInt(str.split(",")[0]));
...
for (let i = 0; i < input.length; i++)
  for (let j = 0; j < input[i].length; j++)
    if (input[i][j] === "*") output += getGearRatio(i, j);
console.log(output);
