...
for (let str of input[1]) {
  ...
  let part = {};
  for (let rating of arr) part[rating[0]] = parseInt(rating.slice(2));
  parts.push(part);
}

function evaluatePart(part, wf = "in") {
  ...
  for (let i = 0; i < workflows[wf].length - 1; i += 4) {
    ...
    if (
      (operator === "<" && part[ratingName] < value) ||
      (operator === ">" && part[ratingName] > value)
    )
      return evaluatePart(part, result);
  }
  return evaluatePart(part, workflows[wf].slice(-1)[0]);
}

let output = 0;
for (const part of parts)
  if (evaluatePart(part) === "A")
    for (let ratingName in part) output += part[ratingName];
console.log(output);
