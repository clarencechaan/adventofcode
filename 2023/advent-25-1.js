...
for (let i = 0; i < input.length; i++) {
  neighbours[input[i][0]] = [];
  for (const neighbour of input[i][1]) neighbours[neighbour] = [];
}

for (let i = 0; i < input.length; i++) {
  const component1 = input[i][0];
  for (const component2 of input[i][1]) {
    neighbours[component1].push(component2);
    neighbours[component2].push(component1);
  }
}

const nodes = Object.entries(neighbours).map((arr) => arr[0]);
const n = Object.entries(neighbours).length;
...
for (const node of nodes) {
  ...
  for (let i = 0; i < queue.length; i++) {
    let frontier = queue[i];
    let wires = new Set();

    for (const fr of frontier)
      for (const ne of neighbours[fr]) if (!seen.has(ne)) wires.add(fr + ne);

    if (wires.size === 3) {
      console.log(seen.size * (n - seen.size));
      return;
    }

    let nextFrontier = [];
    for (const fr of frontier)
      for (const ne of neighbours[fr])
        if (!seen.has(ne)) {
          seen.add(ne);
          nextFrontier.push(ne);
        }

    if (nextFrontier.length) queue.push(nextFrontier);
  }
}
