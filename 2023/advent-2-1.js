let input = `Game 1: 8 green; 5 green, 6 blue, 1 red; 2 green, 1 blue, 4 red; 10 green, 1 red, 2 blue; 2 blue, 3 red
...
function isGamePossible(idx) {
  for (const bag of input[idx]) {
    let sum = { red: 0, green: 0, blue: 0 };
    for (const [count, colour] of bag) sum[colour] += count;
    if (sum.red > 12 || sum.green > 13 || sum.blue > 14) return false;
  }
  return true;
}

let output = 0;
for (let i = 0; i < input.length; i++) if (isGamePossible(i)) output += i + 1;
console.log(output);
