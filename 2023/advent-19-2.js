...
for (let str of input[0]) {
  ...
  let val = str.slice(str.indexOf("{") + 1, -1).split(",");
  for (let i = 0; i < val.length - 1; i++) {
    val[i] = val[i].split(":");
    val[i].splice(0, 1, val[i][0][0], val[i][0][1], val[i][0].slice(2));
  }
  for (let i = 0; i < val.length; i++)
    if (typeof val[i] === "object") val.splice(i, 1, ...val[i]);
  for (let i = 2; i < val.length; i += 4) val[i] = parseInt(val[i]);
  workflows[key] = val;
}

let requirements = { A: [], R: [] };
for (let wf in workflows) requirements[wf] = [];

function buildRequirements(wf = "in", reqs = []) {
  requirements[wf].push([...reqs]);
  if (wf === "A" || wf === "R") return;

  for (let i = 0; i < workflows[wf].length - 1; i += 4) {
    let [ratingName, operator, value, result] = workflows[wf].slice(i, i + 4);
    let oppositeReq = [
      ratingName,
      operator === "<" ? ">" : "<",
      value + (operator === "<" ? -1 : 1),
    ];
    buildRequirements(result, [...reqs, ratingName, operator, value]);
    reqs.push(...oppositeReq);
  }

  buildRequirements(workflows[wf].slice(-1)[0], reqs);
}

buildRequirements();

let ranges = [];
for (const reqs of requirements.A) {
  let range = { x: [1, 4000], m: [1, 4000], a: [1, 4000], s: [1, 4000] };
  for (let i = 0; i < reqs.length; i += 3) {
    let [ratingName, operator, value] = reqs.slice(i, i + 3);
    if (operator === "<" && range[ratingName][1] >= value)
      range[ratingName][1] = value - 1;
    if (operator === ">" && range[ratingName][0] <= value)
      range[ratingName][0] = value + 1;
  }
  ranges.push(range);
}
...
for (let i = 0; i < ranges.length; i++)
  for (let j = i + 1; j < ranges.length; j++) {
    let intersection = {
      x: [
        Math.max(ranges[i].x[0], ranges[j].x[0]),
        Math.min(ranges[i].x[1], ranges[j].x[1]),
      ],
      m: [
        Math.max(ranges[i].m[0], ranges[j].m[0]),
        Math.min(ranges[i].m[1], ranges[j].m[1]),
      ],
      a: [
        Math.max(ranges[i].a[0], ranges[j].a[0]),
        Math.min(ranges[i].a[1], ranges[j].a[1]),
      ],
      s: [
        Math.max(ranges[i].s[0], ranges[j].s[0]),
        Math.min(ranges[i].s[1], ranges[j].s[1]),
      ],
    };

    if (
      intersection.x[0] <= intersection.x[1] &&
      intersection.m[0] <= intersection.m[1] &&
      intersection.a[0] <= intersection.a[1] &&
      intersection.s[0] <= intersection.s[1]
    ) {
      output -=
        (intersection.x[1] - intersection.x[0] + 1) *
        (intersection.m[1] - intersection.m[0] + 1) *
        (intersection.a[1] - intersection.a[0] + 1) *
        (intersection.s[1] - intersection.s[0] + 1);
    }
  }
