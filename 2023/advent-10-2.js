...
let loopCoords = new Set();
function getLoopLength() {
  let x = startX;
  let y = startY;
  let length = 0;
  let direction;
  while (x !== startX || y !== startY || length === 0) {
    loopCoords.add(x + "," + y);
  ...
  if (
    (grid[y][x] === "|" && (direction === "^" || direction === "v")) ||
    (grid[y][x] === "-" && (direction === "<" || direction === ">")) ||
    (grid[y][x] === "L" && (direction === "v" || direction === "<")) ||
    (grid[y][x] === "J" && (direction === "v" || direction === ">")) ||
    (grid[y][x] === "7" && (direction === ">" || direction === "^")) ||
    (grid[y][x] === "F" && (direction === "<" || direction === "^"))
  )
    return length;
  else return 0;
}
...
for (const pipe of pipes) {
  grid[startY][startX] = pipe;
  if (getLoopLength()) break;
}

function enlargeGrid() {
  let result = [];
  for (let i = 0; i < 3 * n; i++) result.push(Array(m * 3).fill(" "));
  for (let i = 0; i < n; i++)
    for (let j = 0; j < m; j++) {
      if (loopCoords.has(j + "," + i)) {
        switch (grid[i][j]) {
          case "|":
            result[i * 3][j * 3 + 1] = "|";
            result[i * 3 + 1][j * 3 + 1] = "|";
          ...
          case "L":
            result[i * 3][j * 3 + 1] = "|";
            result[i * 3 + 1][j * 3 + 1] = "L";
            result[i * 3 + 1][j * 3 + 2] = "-";
            break;
          case "J":
            result[i * 3][j * 3 + 1] = "|";
            result[i * 3 + 1][j * 3 + 1] = "J";
            result[i * 3 + 1][j * 3] = "-";
            break;
          case "7":
            result[i * 3 + 1][j * 3] = "-";
            result[i * 3 + 1][j * 3 + 1] = "7";
            result[i * 3 + 2][j * 3 + 1] = "|";
            break;
          case "F":
            result[i * 3 + 1][j * 3 + 2] = "-";
            result[i * 3 + 1][j * 3 + 1] = "F";
            result[i * 3 + 2][j * 3 + 1] = "|";
        }
      } else result[i * 3 + 1][j * 3 + 1] = ".";
    }

  return result;
}

let enlarged = enlargeGrid();

let queue = [[startX * 3 + 2, startY * 3]];
for (const [x, y] of queue) {
  if (x < 0 || x >= m * 3 || y < 0 || y >= n * 3) continue;
  if (enlarged[y][x] !== "." && enlarged[y][x] !== " ") continue;
  if (enlarged[y][x] === ".") enlarged[y][x] = "I";
  else enlarged[y][x] = "X";
  queue.push([x - 1, y], [x + 1, y], [x, y - 1], [x, y + 1]);
}

let output = 0;
for (const row of enlarged) for (const cell of row) if (cell === "I") output++;
console.log(output);
