...
let orderings = [];
function findOrderings(set = new Set()) {
  if (set.size === 7) {
    orderings.push([...set]);
    return;
  }
  for (let i = 0; i < 7; i++) {
    if (set.has(char[i])) continue;
    set.add(char[i]);
    findOrderings(set);
    set.delete(char[i]);
  }
...
function isOrderingValid(ordering, line) {
  function isWordValid(word) {
    switch (word.length) {
      case 2:
        return word.includes(ordering[2]) && word.includes(ordering[5]);
      case 3:
        return (
          word.includes(ordering[0]) &&
          word.includes(ordering[2]) &&
          word.includes(ordering[5])
        );
      case 4:
        return (
          word.includes(ordering[1]) &&
          word.includes(ordering[2]) &&
          word.includes(ordering[3]) &&
          word.includes(ordering[5])
        );
      case 5:
        return (
          (word.includes(ordering[0]) &&
            word.includes(ordering[2]) &&
            word.includes(ordering[3]) &&
            word.includes(ordering[4]) &&
            word.includes(ordering[6])) ||
          (word.includes(ordering[0]) &&
            word.includes(ordering[2]) &&
            word.includes(ordering[3]) &&
            word.includes(ordering[5]) &&
  ...
  let [left, right] = line;
  return (
    left.every((word) => isWordValid(word)) &&
    right.every((word) => isWordValid(word))
  );
}

function convertWordToDigit(ordering, word) {
  switch (word.length) {
    case 2:
      return 1;
    case 3:
...
for (const line of input) {
  for (const ordering of orderings) {
    if (isOrderingValid(ordering, line)) {
      let number = [];
      for (const word of line[1])
        number.push(convertWordToDigit(ordering, word));
