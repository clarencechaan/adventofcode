...
for (let i = 1; i < polymer.length; i++)
  patternFreq[polymer[i - 1] + polymer[i]] =
    (patternFreq[polymer[i - 1] + polymer[i]] || 0) + 1;

for (let step = 0; step < 40; step++) {
  let toAdd = {};
  for (const [[a, b], c] of insertionRules) {
    let freq = patternFreq[a + b];
    if (freq) {
      freqMap[c] = (freqMap[c] || 0) + freq;
      toAdd[a + c] = (toAdd[a + c] || 0) + freq;
      toAdd[c + b] = (toAdd[c + b] || 0) + freq;
      toAdd[a + b] = (toAdd[a + b] || 0) - freq;
    }
  }
  for (const pattern in toAdd)
    patternFreq[pattern] = (patternFreq[pattern] || 0) + toAdd[pattern];
}
